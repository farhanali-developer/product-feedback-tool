<div>
    <div class="flex mt-10">
        ...
        <!-- Feedback List -->
        <div class="w-1/2 p-4">
            <div class="container px-0 mx-auto sm:px-5" x-data="{ commentResponses: {} }">
                <div class="text-center">
                    <h2 class="text-xl text-gray-800 font-bold sm:text-3xl dark:text-white">
                        All Feedbacks
                    </h2>
                ...
                <div class="flex-col w-full py-4 mx-auto bg-white border rounded-xl sm:mt-10 md:p-10 dark:bg-gray-800 dark:border-gray-700">
                    <div class="flex flex-row">
                        ...
                        <div class="flex-col w-full mt-1 ml-2">
                            <div class="flex items-center flex-1 px-4 font-bold leading-tight dark:text-white">{{ $feedback['user']->name }}
                                <span class="ml-2 text-xs font-normal text-gray-500 dark:text-gray-400">{{ \Carbon\Carbon::parse($feedback->created_at)->diffForHumans() }}</span>
                            </div>
                            
                            <div class="flex-1 px-2 ml-2 text-sm font-medium leading-loose text-gray-600 dark:text-gray-400">Category: <strong class="text-white">{{ ucwords(str_replace('-', ' ', $feedback->category)) }}</strong></div>
                            <div class="flex-1 px-2 ml-2 text-sm font-medium leading-loose text-gray-600 dark:text-gray-400">{{ $feedback->description }}</div>
                            <button class="inline-flex items-center px-1 pt-2 ml-1 flex-column" id="reply-back" wire:ignore wire:key="{{ $feedback->id }}" x-on:click="commentResponses[{{ $feedback->id }}] = !commentResponses[{{ $feedback->id }}]">
                            ...
                            <div x-show="commentResponses[{{ $feedback->id }}]" class="mb-3">
                                <livewire:comment-response :feedback="$feedback" :key="$feedback->id" />
                            </div>
                        </div>
                    </div>

                    @if($feedback->feedbackResponse->isNotEmpty())
                        @foreach($feedback->feedbackResponse as $response)
                            <hr class="my-2 ml-16 border-gray-200">
                            <div class="flex flex-row pt-1 md-10 md:ml-16">
                                <svg class="object-cover w-12 h-12 border-2 border-gray-300 rounded-full" viewBox="0 0 32 32" xmlns="http://www.w3.org/2000/svg"><defs><style>.cls-1{fill:#606161;}</style></defs><title/><g data-name="Layer 7" id="Layer_7"><path class="cls-1" d="M19.75,15.67a6,6,0,1,0-7.51,0A11,11,0,0,0,5,26v1H27V26A11,11,0,0,0,19.75,15.67ZM12,11a4,4,0,1,1,4,4A4,4,0,0,1,12,11ZM7.06,25a9,9,0,0,1,17.89,0Z"/></g></svg>
                                <div class="flex-col mt-1 ml-2">
                                    <div class="flex items-center flex-1 px-4 font-bold leading-tight dark:text-white">
                                        {{ $response['user']->name }}
                                        <span class="ml-2 text-xs font-normal text-gray-500 dark:text-gray-400">{{ \Carbon\Carbon::parse($response->created_at)->diffForHumans() }}</span>
                                    </div>
                                    <div class="flex-1 px-2 ml-2 text-sm font-medium leading-loose text-gray-600 dark:text-gray-400">{{ $response->description }}</div>
                                </div>
                            </div>
                        @endforeach
                    @endif
                ...
                <div class="mt-5">
                    {{-- {{ $feedbacks->links(data: ['scrollTo' => false]) }} --}}
                </div>
            </div>
        </div>
    </div>
